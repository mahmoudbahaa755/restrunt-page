...
var sum = 0;
    const [desertMenu, setDessertMenu] = useState([
 
        {
            ...
            price:4.99,
            component:['Donate'],
            extra:['Choclate','Coffee','White Souce'],
            amount:0
         },
          {
            name:'Chocolate cake',
 ...
 cartCtx.addToCart({
     ...
     price: mainMenu[id].price
            ...
            setMainMenu((prev) => {
                ...
                setMainMenu(()=>{return mainMenu.sort((a,b)=>{return a.id-b.id})})
                setMainMenu((prev) => {
                                        return [...new Set(prev)]
        ...
        if (value<0){
            ...
            setDessertMenu((prev) => {
                ...
                setDessertMenu(()=>{return desertMenu.sort((a,b)=>{return a.id-b.id})})
                setDessertMenu((prev) => {
                                        return [...new Set(prev)]
...
for (let i = 0; i < mainMenu.length; i++) {
    ...
    totalPrice += (mainMenu[i]['amount'] * mainMenu[i]['price']);
}
...
props.getOrderNum(sum)
    ...
    const whichMenuTOShow =  menuSection === true ? (
                                 mainMenu.map((item)=>{
                                return(
                                    <OrderFood key={item.id} menuType={'Main Menu'} AmountChange={AmountChange} 
                                    amount={item.amount}id={item.id} name={item.name} description={item.description} price={item.price} component={item.component}/>
                                )
                            }
                        ) ) : ( desertMenu.map( (item)=>{
                                return(
                                    <OrderFood key={item.id} menuType={ 'Desert Menu'} AmountChange={AmountChange} 
                                    amount={item.amount} id={item.id} name={item.name} description={item.description} price={item.price} component={item.component}/>
                                )
                            }) )
    ...
    return(
                ...
                <Container>
                    ...
                    <button type='button' className={style["button"]} value='Menu' onClick={() => setMenuSection(true)}>Menu</button>
                    <button type='button' className={style["button"]} value='Desert' onClick={() => setMenuSection(false)}>Desert</button>
                    </div>
                    { whichMenuTOShow }
                    </div>
                </Container>
